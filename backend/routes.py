...
def get_latest_oee(line_id):
    ...
    try:
        ...
        if row:
            return jsonify(
                {
                    "OEE": safe_float(row[4]),
                    "A": safe_float(row[1]),
                    "Q": safe_float(row[2]),
                    "P": safe_float(row[3]),
                    "TimeStamp": str(row[6]) if row[6] is not None else None,
                }
            )
        else:
            return jsonify(
                {"OEE": None, "A": None, "P": None, "Q": None, "TimeStamp": None}
            )
    except Exception as e:
...
#             }
#         else:
#             return {"OEE": None, "A": None, "P": None, "Q": None}
#     except Exception as e:
#         return {"error": str(e)}, 500
